...
class CustomAssetCellController: CommonExampleController {
    ...
    @objc func pressedSave(_ sender: Any) {
        ...
        if assets.count < 51 {
            let ac = UIAlertController(title: "Images Selected", message: "Are you sure you want to upload the selected \(self.assets.count) images? If you click save this action cannot be undone.", preferredStyle: .alert)
            let action = UIAlertAction(title: "Yes I am Sure", style: .default, handler: { action in
                for i in 0...self.assets.count - 1 {
                    self.imageManager.requestImage(for: self.assets[i], targetSize: imageSize, contentMode: .aspectFill, options: nil) { image, _ in
                        ...
                        if imagesRemaining > 0 && imagesRemaining <= 50 {
                            ...
                            "/\(albumName)/\(50-imagesRemaining+i+1) of 50 \(Date().getMonthName()) taken on \((self.assets[i].creationDate!).dayMonthYear())"
