...
class CreateAlbumController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let checkoutButton = UIBarButtonItem(title: "Checkout", style: .plain, target: self, action: #selector(submit))
    ...
    @IBAction func addAlbumButtonPressed(_ sender: UIButton) {
        ...
        let todayDay = todayArray[1].dropLast()
    ...
    func addTableRow () {
        let nameSelectionAlert = UIAlertController(title: "Pick a name for your Album", message: nil, preferredStyle: .alert)
...
extension CreateAlbumController: UITableViewDelegate, UITableViewDataSource {
    ...
    @objc private func submit(_ sender: UIBarButtonItem) {
        ...
        self.navigationItem.rightBarButtonItem?.setTitleTextAttributes([NSAttributedString.Key.foregroundColor: UIColor.lightGray], for: .disabled)
            ...
            let ac = UIAlertController(title: "Checkout", message: "Click albums to select them for checkout", preferredStyle: .alert)
            let action = UIAlertAction(title: "Proceed", style: .default, handler: nil)
            ac.addAction(action)
